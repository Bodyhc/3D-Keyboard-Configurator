...
const Keyboard3D: React.FC<Keyboard3DProps> = ({ config }) => {
  ...
  // تحميل الصورة مباشرة باستخدام useTexture بدون useMemo
  const texture = config.backgroundImage
    ? useTexture(config.backgroundImage)
    : undefined;

  const keys =
    LAYOUTS[config.layout as keyof typeof LAYOUTS] || LAYOUTS["60percent"];
  const switchColor =
    SWITCH_COLORS[config.switches as keyof typeof SWITCH_COLORS] ||
    SWITCH_COLORS["cherry-red"];
  const keycapMaterial =
    KEYCAP_MATERIALS[config.keycaps as keyof typeof KEYCAP_MATERIALS] ||
    KEYCAP_MATERIALS["pbt-black"];
  const keyboardColor =
    KEYBOARD_COLORS[config.color as keyof typeof KEYBOARD_COLORS] ||
    KEYBOARD_COLORS["black"];
  ...
  return (
    <group ref={meshRef}>
      ...
      <mesh receiveShadow castShadow position={[0, 0, 0]}>
        <boxGeometry
          args={[
            config.layout === "full" ? 16 : 12,
            0.5,
            config.layout === "60percent" ? 6 : 8,
          ]}
        />
        <meshStandardMaterial
          color={keyboardColor} // اللون الأساسي
          map={texture} // استخدم `map` فقط إذا كان هناك صورة
          metalness={0.6}
          roughness={0.2}
        />
      </mesh>
      ...
      {keys.map((key, index) => (
        <group key={index} position={key.position}>
          ...
          <mesh position={[0, 0.3, 0]}>
            <boxGeometry
              args={[key.size?.[0] || 0.9, 0.2, key.size?.[1] || 0.9]}
            />
